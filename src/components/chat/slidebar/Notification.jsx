...
function Notification() {
  ...
  useEffect(() => {
    const q = query(collection(db, "friend_requests"), where("to", "==", infoUser.uid));
  ...
  const handleAccept = async (friendRequestId, fromUserId) => {
    ...
    setFriends((prevFriends) => prevFriends.filter((friend) => friend.id !== friendRequestId));
  };
      ...
      <Col span={24}>
        <div className="list_notifications">
          <div className="notifications_element">
            {usersData.map((user) => {
              const friendRequests = friends?.find((friend) => friend.from === user.uid && friend.status === "pending");
              return (
                <div key={user.uid}>
                  {friendRequests && (
                    <div className="infoUser_notification">
                      ...
                      <div className="btn_notifications">
                        <Button style={{ margin: "5px" }} type="primary" onClick={() => handleAccept(friendRequests.id, user.uid)}>
                          Xác nhận
                        </Button>
                        <Button style={{ margin: "5px" }} type="default" onClick={() => handleReject(friendRequests.id)}>
                          Từ chối
                        </Button>
                      </div>
                    </div>
